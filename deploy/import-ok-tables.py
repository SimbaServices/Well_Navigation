"""Replace wells_ok / permits_ok / operators_ok from a sidecar SQLite file.

Does not touch Texas, New Mexico, or Louisiana tables.
"""
...
SRC = Path(sys.argv[1] if len(sys.argv) > 1 else "/tmp/ok_tables.db")
DST = Path(sys.argv[2] if len(sys.argv) > 2 else "/app/data/wellnav.db")
TABLES = ("wells_ok", "permits_ok", "operators_ok")
...
def main() -> int:
    ...
    for table in TABLES:
        if not _cols(dst, table):
            print(f"missing destination {table}", flush=True)
            dst.close()
            return 1
