...
class ParseOrganizationResultsTest(unittest.TestCase):
    def test_org_sample_recovers_operator_number_and_name(self) -> None:
        from wellnav.parsers import parse_organization_results

        html = (Path(__file__).resolve().parents[1] / "deploy" / "org-oil-sample.html").read_text(
            encoding="utf-8", errors="replace"
        )
        parsed = parse_organization_results(html)
        self.assertTrue(parsed["pager"])
        self.assertEqual(parsed["total"], 4231)
        by_no = {row["operator_number"]: row for row in parsed["operators"]}
        self.assertEqual(by_no["000016"]["operator_name"], "A & E MINERALS, LLC")
        self.assertGreaterEqual(len(parsed["operators"]), 10)
